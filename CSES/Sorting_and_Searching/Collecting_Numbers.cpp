  ...
  while (!q.empty()) {
    ...
    for (auto child : v[node]) {
      if (m[node] && m[child] && dis[child] > dis[node] + 1) {
        ...
        q.push(child);
      } else if (m[node] && (!m[child] && child < node) &&
                 dis[child] > dis[node]) {
        par[child] = node;
        dis[child] = dis[node];
        q.push(child);
...
void dfs(ll node, vector<vector<ll>> &a, vector<bool> &vis, vector<ll> &dp) {
  vis[node] = true;
  for (ll child : a[node]) {
    if (!vis[child])
      dfs(child, a, vis, dp);
